...
2. **Red.** Write one test for one behavior. Run it. Watch it fail — and read *why* it failed: a failing assertion is red; an import error, typo, or missing fixture is just a broken test. The failure message is the specification talking.
3. **Green.** Write the minimum that makes it pass. Run the covering tests and read the count yourself. Resist implementing the next three tests' worth while you're in there — speculative code is untested code with good posture.
